...
const stateUpdateFromNewAwsCredentials = (
  ...
  const awsConfig = awsCredentials ? { region: appRegion, credentials: awsCredentials } : undefined;
      ...
      fetch(appAuthUrl, {
        ...
        .then((response) => {
          response
            .json()
            .then((res) => {
              loginWithAwsCognitoIdentityPoolSemDispatch(appConfig, res.id_token, res.access_token)
                .then((stateUpdate) => {
                  resolve(stateUpdate);
                })
                .catch((err) => {
