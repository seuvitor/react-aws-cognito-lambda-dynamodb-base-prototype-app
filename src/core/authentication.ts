...
const getRefreshTokenFromSessionStorage = (appRefreshTokenStorageKey) => (
  sessionStorage.getItem(appRefreshTokenStorageKey)
);

const updateRefreshTokenInSessionStorage = (refreshToken, appRefreshTokenStorageKey) => {
  if (refreshToken) {
...
const stateUpdateFromNewAwsCredentials = (
  ...
  const user = {
    identityId,
    id: idTokenPayload && idTokenPayload.sub,
    name: idTokenPayload ? idTokenPayload.name : appMessages.LOGGED_OUT_USER,
    email: idTokenPayload && idTokenPayload.email,
    groups: idTokenPayload && idTokenPayload['cognito:groups'],
    idToken,
    accessToken
  };
...
const loginWithAwsCognitoIdentityPoolSemDispatch = (
  ...
  return new Promise((resolve, reject) => {
    newCredentials().then((creds) => {
      resolve(stateUpdateFromNewAwsCredentials(
        newCredentials,
        creds.identityId,
        idToken,
        accessToken,
...
const loginWithAuthorizationCodeSemDispatch = (
  ...
  return new Promise((resolve, reject) => {
    fetch(appAuthUrl, {
      ...
      response.json().then((res) => {
        refreshIdAndAccessTokensSemDispatch(
          appConfig,
          res.refresh_token
        ).then((stateUpdate) => {
          resolve({
...
const logoff = (
  ...
  new Promise((resolve, _reject) => {
    updateRefreshTokenInSessionStorage(undefined, appConfig.appRefreshTokenStorageKey);
...
const setAppConfig = (
  ...
  const refreshToken = getRefreshTokenFromSessionStorage(appConfig.appRefreshTokenStorageKey);
  if (refreshToken) {
...
const loginWithAuthorizationCode = (
  ...
  new Promise((resolve, reject) => {
    loginWithAuthorizationCodeSemDispatch(
      appConfig,
      authorizationCode
    ).then((stateUpdate) => {
      const { refreshToken } = stateUpdate;
      updateRefreshTokenInSessionStorage(refreshToken, appConfig.appRefreshTokenStorageKey);
