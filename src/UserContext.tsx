...
const UserProvider = ({ children }: PropsWithChildren) => {
  ...
  const logoff = useCallback(() => _logoff(setUserState, appConfig), [appConfig]);

  const loginWithAwsCognitoIdentityPool = useCallback(
    (idToken: string | undefined, accessToken: string | undefined) =>
      _loginWithAwsCognitoIdentityPool(setUserState, appConfig, idToken, accessToken),
    [appConfig],
  );

  const refreshIdAndAccessTokens = useCallback(
    () => _refreshIdAndAccessTokens(setUserState, appConfig, userState.refreshToken),
...
const useUser = () => {
  const { user, awsConfig, awsCredentials, loginAnonymously, loginWithAuthorizationCode, logoff } =
