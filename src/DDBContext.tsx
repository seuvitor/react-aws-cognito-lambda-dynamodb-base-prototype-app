...
const useDDB = <T,>(): UseDDBReturnType<T> => {
  ...
  const ddbGet = useCallback(
    (params: GetCommandInput) =>
      documentDB
        ? documentDB.send(new GetCommand(params)).then((output) => output as DDBGetCommandOutput<T>)
  ...
  const ddbPut = useCallback(
    (params: PutCommandInput) =>
      documentDB ? documentDB.send(new PutCommand(params)) : Promise.reject<PutCommandOutput>(),
    [documentDB],
  );
